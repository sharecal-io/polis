...
export class DirectoryUsers {
  ...
  public async create(directory: Directory, body: any): Promise<DirectorySyncResponse> {
    ...
    // Dedupe on userName (RFC 7643: uniqueness=server, caseExact=false) rather than
    // emails[0].value (uniqueness=none). emails[0].value can lag userName during
    // IdP-driven renames, producing false 409s on a deactivated record.
    const { data: users } = await this.users.search(body.userName, directory.id);

    if (users && users.length > 0) {
      // RFC 7644 §3.3 / §3.12: a uniqueness conflict on POST must include
      // scimType: "uniqueness" so the IdP knows to retry as a PATCH on the
      // existing resource instead of looping POST.
  ...
  private respondWithError(error: (ApiError & { scimType?: string }) | null) {
    return {
      ...
      data: {
        ...
        ...(error && 'scimType' in error && error.scimType ? { scimType: error.scimType } : {}),
